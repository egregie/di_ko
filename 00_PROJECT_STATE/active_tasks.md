...
- [~] Build Layout Library v2: bounds + relative_bounds + max_chars [Phase 8.1] → DELIVERED via Phase 8.2 (4-class Authority system, 13 layouts) instead of donor-hd_1 mirrored *_left/_right
- [x] Integrate 3 client logos: #000→currentColor (0 residual black), deprecate old, registry+provenance [Phase 8.2 Block A]
- [x] layouts_v2.json: 13 layouts / 4 classes / slot-geometry @1280×720 + anti-fabrication policy (gen_layouts_v2.py) [Phase 8.2 Block B/D1]
- [x] deck_templates.json: 2 carcasses (Retinoids 16, Postacne 18), composition without content [Phase 8.2 Block C/D2]
- [x] render_carcass.py: both empty carcasses → real PDF (sage plates, brand chrome, Zero Black, no literal [Placeholder]) [Phase 8.2 Block D3]
- [x] validate_layouts.py gate (negative-control proven); validate_graph stays PASS [Phase 8.2 Block E]
- [x] Client reviewed carcasses → approved structure, flagged 3 layouts (A2/C1/C3/D3) + refreshed logo set [Phase 8.2 → 8.3]
- [x] Block A: reference analysis (3 ref decks + old) + 21 mini-SVG classified/recolored + slide→fact→grade map [Phase 8.3]
- [x] Block B: rework A2 (2×2 grid), C1/C3 (plate+interp+grade), D3 (summary+badge); B3 kept; validate_layouts PASS [Phase 8.3]
- [x] Block C: mini-SVG icons in text + bg-molecule (0.14) on text slides + logo-presence logic in renderer [Phase 8.3]
- [x] Block D: 41 factual blocks graph-traced (fact_id+grade) + author frames + grade chips; deck_*_provenance.json [Phase 8.3]
- [x] Block E: 06_render hygiene (templates/ + content/ + deprecated/) [Phase 8.3]
- [x] Block F: validate_content gate (negative-control proven) + adversarial fidelity 2 rounds → 7 fixes; both decks → PDF; validate_graph PASS [Phase 8.3]
- [ ] Confirmatory 3-skeptic majority-vote fidelity re-run (subagent quota exhausted; optional, not a blocker) [Phase 8.3 backlog]
- [ ] Diagram-engine phase: fill id_graph/id_illustration slots (tables/schemes/charts) from graph [Phase 8.4+]
- [ ] Client review of the 2 filled decks (06_render/content/deck_*_filled.pdf) [Phase 8.3 — PENDING CLIENT]
