...
- **Gemini quota / rate limit (updated 2026-06-12)**: the free-tier judge key enforces a tight per-minute rate limit (≈1 call per window) plus a daily cap. `evidence.py` now backs off on the 429 `retryDelay` (≤65s, 6 attempts) instead of bailing on the "PerDay" substring (DEC-023), so per-minute limits are waited out; once the daily cap is exhausted even backoff fails → NEEDS_MANUAL. ~~Deferred on quota: fact_0076/0077/0079, fact_0074, fact_0015.~~ **RESOLVED 2026-06-12** (quota reset): all re-judged and integrated (0076 WEAK, 0077/0079/0074 SUPPORTED, 0015 WEAK grounded). Pace heavy batches; a paid key removes the per-window limit.
- **Windows console UTF-8 (RESOLVED 2026-06-12, DEC-024)**: `verify_gate.py` now forces UTF-8 stdout/stderr. Previously the judge's verbatim Unicode quotes crashed `print()` on the legacy code page, was swallowed by the generic handler, and a real verdict was reported as NEEDS_MANUAL (hit fact_0079). Any new gate-calling script should keep this guard. **Open revision item**: facts that were quarantined as NEEDS_MANUAL/UNSUPPORTED *before* this fix and whose supporting abstract sentence contains Unicode (≤, ∼, ×, °, β) may have been false-dropped by this crash, not by a genuine verdict. When convenient, re-run the rejected/ pool through the now-UTF-8 gate and restore any that come back SUPPORTED/WEAK with a grounded quote.
- **Grade-gate inflation hole (RESOLVED 2026-06-12, DEC-025)**: `evidence_ok`/`a_keywords` granted EBM grade A to any "journal article"/"review"/"comparative study" pub-type. Audit of all 55 active facts found **32 (58%) over-graded**; corrected downgrade-only (now A:22 / B:2 / C:31) with `audit_flag=grade_corrected`. Root cause fixed via `evidence.derive_grade` + a `validate_graph` grade-consistency check. Note for decks: ~31 facts are now honestly grade C (narrative review / case series / uncontrolled / in-vitro) — they remain valid and deck-eligible (only D is barred from clinical slides) but must be framed as such, not as RCT-grade evidence.
