...
POSTACNE = {
 ...
 4: {"part_label": F("Часть 1"), "section_title": F("Пигментация"),
     "section_subtitle": F("Природа, активы, доказательная база"), "logo": LOGO("mark")},
 5: {"title": F("PIH и PIE — разная природа, разная тактика"),
     ...
     "label_3": FACT("fact_0065", short="Обзор: при PIH сильнее всего доказаны ретиноиды и гидроксикислоты; рекомендован SPF",
                     icon="mini_10")},
 ...
 9: {"part_label": F("Часть 2"), "section_title": F("Рубцы"),
     "section_subtitle": F("Типы рубцов и выбор процедуры"), "logo": LOGO("mark")},
 ...
 11:{"title": F("Карта выбора процедуры по типу рубца"),
     ...
     "branch_2": LIST(F("Boxcar / общие →", icon="mini_04"),
                      FACT("fact_0067", short="Микронидлинг (систем. обзор РКИ): эффективен и безопасен при атроф. рубцах постакне"),
 ...
 12:{"title": F("Глубина воздействия процедур по слоям кожи"),
     ...
     "label_1": FACT("fact_0075", short="Субцизия — хирургический метод при атрофических рубцах постакне", icon="mini_19"),
     "label_2": FACT("fact_0078", short="Абляционный фракц. CO₂-лазер — эффективен при атрофических рубцах постакне", icon="mini_04"),
     "label_3": FACT("fact_0079", short="Фракц. RF — предпочтительна при склонности к пигментации", icon="mini_07")},
 13:{"title": F("Субцизия · лазер · RF-микронидлинг"),
     ...
     "interp": LIST(F("Сильные стороны и доказательная база:"),
                    FACT("fact_0078", short="CO₂-лазер: эффективен при атрофических рубцах постакне"),
                    FACT("fact_0067", short="Микронидлинг: безопасен, хорошо переносится (обзор РКИ)"),
                    FACT("fact_0076", short="Субцизия: безопасный и эффективный метод при атроф. рубцах постакне")),
...
RETINOIDS = {
 ...
 9: {"title": F("Третиноин · адапален · тазаротен"),
     ...
     "interp": LIST(F("Профиль каждого актива:"),
                    FACT("fact_0005", short="Третиноин — золотой стандарт фотостарения (по обзорным данным)"),
 ...
 11:{"part_label": F("Часть 3"), "section_title": F("Решения и применение"),
     "section_subtitle": F("Выбор по задаче и безопасность"), "logo": LOGO("mark")},
 12:{"title": F("Выбор ретиноида по задаче"),
     "root": F("Задача определяет молекулу и силу"),
     "branch_1": LIST(F("Фотостарение →", icon="mini_03"),
                      FACT("fact_0005", short="Третиноин — золотой стандарт фотостарения (по обзорам)"),
