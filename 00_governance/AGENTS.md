...
- **P011 Deck Readiness**: A cosmetic topic qualifies for a presentation slide deck only when it has at least 8 verified, clean facts in the active knowledge graph. Presentations for topics below this threshold are deferred until more facts are ingested under the verification gate. **Scope (Phase 8.7b):** the ≥8 threshold applies only to `topical_active` and `procedure` topics. Auxiliary node-types `clinical_condition` (e.g. acne, photoaging — antecedent conditions) and `contraindication_context` (e.g. pregnancy, lactation) are graph nodes, not deck-ready topics, and are exempt from the threshold.
